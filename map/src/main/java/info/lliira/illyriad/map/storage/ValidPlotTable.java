...
public class ValidPlotTable extends LocationTable<ValidPlot, ValidPlot.Builder> {
  private static final List<Field<ValidPlot, ValidPlot.Builder, ?>> DATA_FIELDS =
      List.of(
          new Field<>("total_sum", FieldType.INT, v -> v.totalSum, ValidPlot.Builder::totalSum),
          new Field<>("food_sum", FieldType.INT, v -> v.foodSum, ValidPlot.Builder::foodSum),
          new Field<>(
              "sov_count", FieldType.INT, v -> v.sovereignCount, ValidPlot.Builder::sovereignCount),
          new Field<>(
              "restricted", FieldType.BOOLEAN, v -> v.restricted, ValidPlot.Builder::restricted));
  ...
  ValidPlotTable(Connection connection) {
    ...
    try {
      selectCandidateStatement =
          connection.prepareStatement(
              "SELECT v.* FROM valid_plots v, plots p "
  ...
  public Iterator<ValidPlot> selectCandidates(
      int foodThreshold, int totalThreshold, int sovereignThreshold) throws SQLException {
