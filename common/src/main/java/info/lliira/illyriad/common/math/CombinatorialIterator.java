package info.lliira.illyriad.common.math;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import java.util.NoSuchElementException;

public class CombinatorialIterator<E> implements Iterator<List<E>>, Iterable<List<E>> {

  private int[] indices;
  private final int[] maxIndices;
  private final List<E> elements;

  public CombinatorialIterator(int number, List<E> items) {
    if (items == null || number < 1 || items.size() < number) {
      throw new IllegalArgumentException("|items| >= number && number > 1");
    }
    elements = items;
    indices = new int[number];
    maxIndices = new int[number];
    for (int i = 0; i < number; i++) {
      indices[i] = i;
      maxIndices[i] = elements.size() + i - indices.length;
    }
  }

  public Iterator<List<E>> iterator() {
    return this;
  }

  public boolean hasNext() {
    return indices != null;
  }

  private List<E> createFromIndices() {
    List<E> result = new ArrayList<>(indices.length);
    for (int i : indices) {
      result.add(elements.get(i));
    }
    return result;
  }

  private void incrementIndices() {
    if (indices[0] == maxIndices[0]) {
      indices = null;
      return;
    }
    for (int i = indices.length - 1; i >= 0; i--) {
      if (indices[i] != maxIndices[i]) {
        int val = ++indices[i];
        for (int j = i + 1; j < indices.length; j++) {
          indices[j] = ++val;
        }
        break;
      }
    }
  }

  public List<E> next() {
    if (indices == null) {
      throw new NoSuchElementException("End of iterator");
    }
    List<E> result = createFromIndices();
    incrementIndices();
    return result;
  }

  public void remove() {
    throw new UnsupportedOperationException("remove");
  }
}
